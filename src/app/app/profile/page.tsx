...
/**
 * Render the user profile page with controls to view, edit, and save personal, physical, and preference information.
 *
 * @returns A React element representing the profile page UI.
 */
export default function ProfilePage() {
  ...
  const fetchProfile = useCallback(async () => {
    try {
      ...
      // Auth is now handled by the layout
      if (!user) {
        setMessage({ type: "error", text: "Unable to load user information" })
        setLoading(false)
        return
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Auth is now handled by the layout
      if (!user) {
        logError(new Error('No authenticated user'), 'profile_update')
        setMessage({ type: "error", text: "Unable to load user information" })
        setSaving(false)
        return
      }
